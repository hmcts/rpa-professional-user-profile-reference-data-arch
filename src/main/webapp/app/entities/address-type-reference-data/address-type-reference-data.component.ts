...
@Component({
    selector: 'jhi-address-type-reference-data',
    templateUrl: './address-type-reference-data.component.html'
})
export class AddressTypeReferenceDataComponent implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    loadAll() {
        this.addressTypeService.query({
            ...
            sort: this.sort()}).subscribe(
                (res: HttpResponse<AddressTypeReferenceData[]>) => this.onSuccess(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/address-type-reference-data'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/address-type-reference-data', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: AddressTypeReferenceData) {
        return item.id;
    }

...
@Component({
    selector: 'jhi-organisation-reference-data',
    templateUrl: './organisation-reference-data.component.html'
})
export class OrganisationReferenceDataComponent implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    loadAll() {
        this.organisationService.query({
            ...
            sort: this.sort()}).subscribe(
                (res: HttpResponse<OrganisationReferenceData[]>) => this.onSuccess(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/organisation-reference-data'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/organisation-reference-data', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: OrganisationReferenceData) {
        return item.id;
    }
